...
// GET /api/tasks/category/[category] - Return tasks filtered by category with user's latest submission and status
export async function GET(
  ...
  try {
    ...
    const totalExpiredTasks = tasks.filter(task => task.status === 'expired').length;
    const totalPending = tasksWithStatus.filter(task => task.userTaskStatus === 'pending').length;
    const totalApproved = tasksWithStatus.filter(task => task.userTaskStatus === 'approved').length;
